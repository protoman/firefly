...
void projectilePreviewArea::paintEvent(QPaintEvent *) {
    ...
    if (GameData::get_instance()->projectile_list_v0.size() < 1) {
        return;
    }

	std::string projectile_filename;
    std::string graphic_filename(GameData::get_instance()->projectile_list_v0.at(Mediator::get_instance()->current_projectile).graphic_filename);
    if (graphic_filename.length() == 0) {
        ...
        projectile_filename = SharedData::get_instance()->FILEPATH + "/images/projectiles/" + GameData::get_instance()->projectile_list_v0.at(Mediator::get_instance()->current_projectile).graphic_filename;
	}
   ...
   int step = GameData::get_instance()->projectile_list_v0.at(Mediator::get_instance()->current_projectile).size.width*2;
   ...
   max = this->height()+1;
   step = GameData::get_instance()->projectile_list_v0.at(Mediator::get_instance()->current_projectile).size.height*2;
   if (step < 1) {
       step = 1;
   }
   for (i=0; i<max; i=i+step) {
