...
void AreaEditTab::on_currentStageComboBox_currentIndexChanged(int index)
{
    ...
    fill_area_combo();
    ui->areaNameLineEdit->setText(SharedData::get_instance()->v6_area_map.at(SharedData::get_instance()->v6_selected_stage).at(SharedData::get_instance()->v6_selected_area).area_name);
...
void AreaEditTab::fill_data()
{
    ...
    for (unsigned int i=0; i<SharedData::get_instance()->v6_stage_list.size(); i++) {
        ui->currentStageComboBox->addItem(QString("[") + QString::number(i) + "] - " + QString(SharedData::get_instance()->v6_stage_list.at(i).stage_name));
    }
    fill_area_combo();
...
void AreaEditTab::set_data()
{
    ...
    if (currentMap >= SharedData::get_instance()->v6_stage_list.size()) {
        return;
    }
    ...
    ui->stageNameLineEdit->setText(SharedData::get_instance()->v6_stage_list.at(currentMap).stage_name);
    ...
    ui->musicFilenameComboBox->setCurrentIndex(ui->musicFilenameComboBox->findText(SharedData::get_instance()->v6_stage_list.at(currentMap).music_filename));
...
int AreaEditTab::get_area_style_number()
{
    ...
    if (it->second.at(SharedData::get_instance()->v6_selected_area).style >= SharedData::get_instance()->v6_style_list.size()) {
...
void AreaEditTab::on_stageNameLineEdit_textEdited(const QString &arg1)
{
    ...
    sprintf(SharedData::get_instance()->v6_stage_list.at(ui->currentStageComboBox->currentIndex()).stage_name, "%s", arg1.toStdString().c_str());
    //fill_data();
...
void AreaEditTab::on_addAreaPushButton_clicked()
{
    ...
    int list_size = SharedData::get_instance()->v6_stage_list.size();
    sprintf(SharedData::get_instance()->v6_stage_list.at(list_size-1).stage_name, "STAGE [%d]", list_size);
    ui->currentStageComboBox->addItem(QString("[") + QString::number(list_size) + "] - " + QString(SharedData::get_instance()->v6_stage_list.at(list_size-1).stage_name));
...
{
    if (checkLoadingAndValid()) { return; }
    sprintf(SharedData::get_instance()->v6_stage_list.at(ui->currentStageComboBox->currentIndex()).music_filename, "%s", arg1.toStdString().c_str());
}
...
void AreaEditTab::on_styleComboBox_currentIndexChanged(int index)
{
    ...
    if (index >= SharedData::get_instance()->v6_style_list.size()) {
...
{
    SharedData::get_instance()->add_missing_areas();
    SharedData::get_instance()->v6_area_map.at(SharedData::get_instance()->v6_selected_stage).push_back(file_v6_area());
    int list_size = SharedData::get_instance()->v6_area_map.at(SharedData::get_instance()->v6_selected_stage).size();
    sprintf(SharedData::get_instance()->v6_area_map.at(SharedData::get_instance()->v6_selected_stage).at(list_size-1).area_name, "AREA [%d]", list_size);
    ui->areaSelectionComboBox->addItem(QString("[") + QString::number(list_size) + "] - " + QString(SharedData::get_instance()->v6_area_map.at(SharedData::get_instance()->v6_selected_stage).at(list_size-1).area_name));
}
