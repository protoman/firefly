...
animTileEdit::animTileEdit(QWidget *parent) : QWidget(parent), graphic_filename(""), img_grid_w(16), img_grid_h(16)
{
    myParent = parent;
...
void animTileEdit::set_graphicfile(std::string filename)
{
    graphic_filename = filename;
}

std::string animTileEdit::get_graphicfile()
{
    return graphic_filename;
}

void animTileEdit::set_bg_graphicfile(std::string filename)
{
    _bg_graphic_filename = filename;
}

void animTileEdit::set_grid_w(int new_w)
{
    img_grid_w = new_w;
    repaint();
}

void animTileEdit::set_grid_h(int new_h)
{
    img_grid_h = new_h;
    repaint();
}

void animTileEdit::set_sprite_pos(st_position s_pos)
{
...
void animTileEdit::paintEvent(QPaintEvent *) {
    ...
    if (bg_image.isNull() == false && bg_image.width() > 0) {
        ...
        bg_image = bg_image.scaled(bg_image.width()*ADJUSTED_SIZE, bg_image.height()*ADJUSTED_SIZE);
    }
    ...
    for (i=0; i<=image.width()/SHOW_TILESIZE; i++) {
        // linhas verticais
        line = QLineF(i*SHOW_TILESIZE, 0, i*SHOW_TILESIZE, SHOW_TILESIZE);
        painter.drawLine(line);
    }
    for (i=0; i<2; i++) {
        // linhas horizontais
        line = QLineF(0, i*SHOW_TILESIZE, image.width(), i*SHOW_TILESIZE);
        painter.drawLine(line);
    }
...
void animTileEdit::mousePressEvent(QMouseEvent *event) {
...
int animTileEdit::get_cursor_pos()
{
