...
void file_io::write_game(file_game& data_in) {
    FILE *fp;
    std::string filename = get_game_properties_filename();
    ...
    fwrite(&data_in, sizeof(struct file_game), 1, fp);
    fclose(fp);
...
std::string file_io::get_game_properties_filename()
{
    return std::string(SharedData::get_instance()->FILEPATH) + "/game_properties" + sufix + ".dat";
}


bool file_io::file_exists(std::string filename)
{
...
{
    fio_common fio_cmm;
    std::map<int, v6_file_game_object_state>::iterator it;
    // convert map to list
    std::vector<v6_file_game_object_state> list;
    for (it=SharedData::get_instance()->game_object_state_map.begin(); it != SharedData::get_instance()->game_object_state_map.end(); it++) {
...
void file_io::load_game_object_state()
{
    fio_common fio_cmm;
