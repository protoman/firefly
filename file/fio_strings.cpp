...
bool fio_strings::file_exists(std::string filename)
{
    bool res = false;
    FILE *fp;
...
std::string fio_strings::get_stage_dialogs_filename(short stage_id, int language)
{
    ...
    std::string filename = SharedData::get_instance()->FILEPATH + std::string(char_filename);
    return filename;
}
...
{
    if (SharedData::get_instance()->FILEPATH == "") {
...
std::vector<std::string> fio_strings::get_string_list_from_scene_text_file(int text_scene_n, int language)
{
    ...
    */
    ...
    std::string filename = SharedData::get_instance()->FILEPATH + "scenes/text/" + get_language_filename_prefix(language) + "/" + std::string(file_chr);
    filename = StringUtils::clean_filename(filename);
    ...
    if (!file_exists(filename) && language != LANGUAGE_ENGLISH) {
        ...
        filename = SharedData::get_instance()->FILEPATH + "scenes/text/" + get_language_filename_prefix(LANGUAGE_ENGLISH) + "/" + std::string(file_chr);
        filename = StringUtils::clean_filename(filename);
    }
    if (file_exists(filename)) {
