...
std::map<int, int> fio_scenes::load_game_scenes()
{
    std::map<int, int> res;
    std::vector<st_game_scene_item> list = load_from_disk<st_game_scene_item>("game_scenes.dat");
    for (int i=0; i<list.size(); i++) {
        res.insert(std::pair<int, int>(list.at(i).id, list.at(i).scene_id));
    }
    return res;
}
...
void fio_scenes::save_game_scenes(std::map<int, int> data)
{
    std::vector<st_game_scene_item> data_vector;
    for (std::map<int, int>::iterator itr = data.begin(), itr_end = data.end(); itr != itr_end; ++itr) {
...
void fio_scenes::generate_files()
{
    ...
    save_game_scenes(std::map<int, int>());
