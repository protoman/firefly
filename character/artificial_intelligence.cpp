...
std::vector<character*> *artificial_intelligence::player_list=nullptr;


artificial_intelligence::artificial_intelligence() :  walk_range(TILESIZE*6), target(nullptr), speed_y(max_speed), acceleration_y(0.05), _ai_timer(0), _ai_chain_n(0), _trajectory_parabola(nullptr)
{
...
void artificial_intelligence::execute_ai()
{
...
bool artificial_intelligence::execute_special_ai()
{
    if (name == "Rotate Test") {
        boss_001_execute_ai();
        return true;
...
void artificial_intelligence::show()
{
    //std::cout << "artificial_intelligence::show" << std::endl;
    if (name == "Rotate Test") {
        boss_001_show();
    } else {
        character::show();
...
void artificial_intelligence::hit_player()
{
    did_hit_player = true;
}

void artificial_intelligence::check_ai_reaction()
{
...
void artificial_intelligence::define_ai_next_step()
{
    bool must_ignore_next = (shot_success == false && _current_ai_type == AI_ACTION_SHOT_PROJECTILE_AHEAD);
        ...
        /// TODO: use some kind of type instead of name ///
...
void artificial_intelligence::execute_ai_step()
{
    //std::cout << "artificial_intelligence::execute_ai_step[" << name << "] - _number: " << (int)_number << ", _current_ai_type: " << (int)_current_ai_type << std::endl;
    ...
    if (is_shooter) {
        if (shooter_timer < TimerView::get_instance()->getTimer()) {
            throw_projectile(0, false);
            shooter_timer = TimerView::get_instance()->getTimer() + SHOOTER_DELAY;
        }
    }
}

void artificial_intelligence::execute_play_sfx()
{
    // TODO: add options
    SoundView::get_instance()->play_shared_sfx("dinosaur_growl.wav");
    _ai_state.sub_status = IA_ACTION_STATE_FINISHED;
}

void artificial_intelligence::execute_ai_wall_walk()
{
    // INITIALIZATION
    if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
...
// ********************************************************************************************** //
struct_player_dist artificial_intelligence::dist_npc_players()
{
    int dist;
    ...
    if (GameManager::get_instance()->get_player() == nullptr) {
    ...
    res.pObj = GameManager::get_instance()->get_player();
...
void artificial_intelligence::ground_damage_players()
{
    // check if player is on ground
    st_float_position npc_pos = GameManager::get_instance()->get_player()->getPosition();
    npc_pos.x = (npc_pos.x + GameManager::get_instance()->get_player()->get_size().width/2)/TILESIZE;
    npc_pos.y = (npc_pos.y + GameManager::get_instance()->get_player()->get_size().height)/TILESIZE;
    ...
    if (lock != TERRAIN_UNBLOCKED && lock != TERRAIN_STAIR && lock != TERRAIN_WATER) {
        ...
        GameManager::get_instance()->get_player()->damage(3, false);
    }
}

void artificial_intelligence::push_back_players(short direction)
{
    GameManager::get_instance()->get_player()->push_back(direction);
}

void artificial_intelligence::pull_players(short direction)
{
    GameManager::get_instance()->get_player()->pull(direction);
}

bool artificial_intelligence::auto_respawn() const
{
...
void artificial_intelligence::ia_action_jump_to_player()
{
    if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
...
void artificial_intelligence::ia_action_jump_to_point(st_position point)
{
    int xinc = 0;
    if (state.direction == ANIM_DIRECTION_LEFT) {
...
void artificial_intelligence::ia_action_jump_to_random()
{
    if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
        _ai_state.sub_action_sub_status = IA_ACTION_STATE_INITIAL;
        int dist = 0;
...
void artificial_intelligence::ia_action_jump_ahead()
{
    if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
...
void artificial_intelligence::ia_action_jump_once()
{
    if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
...
void artificial_intelligence::ia_action_jump_up()
{
    if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
        _ai_state.sub_status = IA_ACTION_STATE_EXECUTING;
...
void artificial_intelligence::ia_action_jump_to_roof()
{
	if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
        set_animation_type(ANIM_TYPE_JUMP);
		_ai_state.sub_status = IA_ACTION_STATE_EXECUTING;
...
void artificial_intelligence::ia_action_air_walk()
{

	if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
		//std::cout << "artificial_intelligence::ia_action_air_walk - INITIALIZE" << std::endl;
        set_animation_type(ANIM_TYPE_WALK_AIR);
		_ai_state.sub_status = IA_ACTION_STATE_EXECUTING;
		struct_player_dist dist_npc_player = dist_npc_players();
        _dest_point.x = dist_npc_player.pObj->getPosition().x;
		//std::cout << "artificial_intelligence::ia_action_air_walk - dist:" << dist_npc_player.dist << std::endl;
	} else if (_ai_state.sub_status == IA_ACTION_STATE_EXECUTING) {
		//std::cout << "artificial_intelligence::ia_action_air_walk - EXECUTE" << std::endl;
...
void artificial_intelligence::ia_action_jump_fall()
{
    //std::cout << "******** artificial_intelligence::ia_action_jump_fall - speed_y: " << speed_y << std::endl;
	if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
		_ai_state.sub_status = IA_ACTION_STATE_EXECUTING;
        set_animation_type(ANIM_TYPE_JUMP);
        speed_y = 5;
...
void artificial_intelligence::ia_action_quake_attack()
{
	if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
		_ai_state.sub_status = IA_ACTION_STATE_EXECUTING;
...
void artificial_intelligence::ia_accelerate_up()
{
...
void artificial_intelligence::ia_accelerate_down()
{
...
void artificial_intelligence::ia_set_destiny_point(st_position dest_point)
{
	_ai_state.initial_position.x = position.x;
	_ai_state.initial_position.y  = position.y;
...
// @TODO - jump if needed
void artificial_intelligence::ia_walk_to_position()
{
...
void artificial_intelligence::ia_float_to_position()
{
	bool finished = false;
	int xinc = 0;
	int yinc = 0;
	...
	if (xinc == 0 && yinc == 0) { // both X and Y blocked, leave
		...
		std::cout << "*** artificial_intelligence::ia_float_to_position - xinc: " << xinc << ", yinc: " << yinc << std::endl;
		position.x += xinc;
		position.y += yinc;
	}
...
void artificial_intelligence::ia_dash()
{
	bool finished = false;
	if (_ai_state.secondary_position.x < _ai_state.initial_position.x) {
...
/// @TODO - jump if needed
void artificial_intelligence::execute_ai_step_walk()
{
	...
	if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
		...
		_ai_state.sub_status = IA_ACTION_STATE_EXECUTING;
        //if (name == "KURUPIRA BOT") std::cout << ">>>>>>> artificial_intelligence::execute_ai_step_walk - direction" << (int)state.direction << std::endl;
	} else {
...
void artificial_intelligence::execute_ai_action_wait_until_player_in_range()
{
	if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
		_ai_state.sub_status = IA_ACTION_STATE_EXECUTING;
...
void artificial_intelligence::execute_ai_action_trow_projectile(unsigned short n, bool invert_direction)
{
	if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
        ...
        if (_was_animation_reset == true && _did_shot == true) {
            ...
            _did_shot = true;
            shot_timer = TimerView::get_instance()->getTimer();
        }
    }
}

// creates a projectile, return false if could not fire
bool artificial_intelligence::throw_projectile(int projectile_type, bool invert_direction)
{
...
void artificial_intelligence::execute_ai_step_fly()
{
    ...
    // EXECUTION
        ...
        if (state.animation_type == ANIM_TYPE_TURN && have_frame_graphic(state.direction, state.animation_type, (state.animation_state+1)) == false) {
            //std::cout << "*************** artificial_intelligence::execute_ai_step_fly - reached last turn frame, reset to stand" << std::endl;
        ...
        if (_parameter == AI_ACTION_FLY_OPTION_HORIZONTAL_AHEAD) {
            //std::cout << "artificial_intelligence::execute_ai_step_fly - HORIZONTAL RIGHT" << std::endl;
            if (move_to_point(_dest_point, move_speed, 0, is_ghost, false) == true) {
            ...
            if (state.animation_type != ANIM_TYPE_TURN) {
                if (name == "Giant Fly") std::cout << "artificial_intelligence::execute_ai_step_fly - HORIZONTAL TURN" << std::endl;
			...
			//std::cout << "artificial_intelligence::execute_ai_step_fly - UP" << std::endl;
            if (move_to_point(_dest_point, 0, move_speed, is_ghost, false) == true) {
                _ai_state.sub_status = IA_ACTION_STATE_FINISHED;
            }
        } else if (_parameter == AI_ACTION_FLY_OPTION_DOWN) {
            //std::cout << "artificial_intelligence::execute_ai_step_fly - DOWN - dest_point.y: " << _dest_point.y << std::endl;
            if (move_to_point(_dest_point, 0, move_speed, is_ghost, false) == true) {
                _ai_state.sub_status = IA_ACTION_STATE_FINISHED;
            }
        ...
        */
            ...
            //std::cout << "artificial_intelligence::execute_ai_step_fly - POSITION[" << position.x << ", " << position.y << "], POINT[" << _dest_point.x << ", " << _dest_point.y << "]" << std::endl;
            if (move_to_point(_dest_point, move_speed, move_speed, is_ghost, false) == true) {
                //std::cout << "artificial_intelligence::execute_ai_step_fly: FINISHED" << std::endl;
                _ai_state.sub_status = IA_ACTION_STATE_FINISHED;
            }
            ...
            if (move_to_point(_saved_point, move_speed, move_speed, is_ghost, false) == true) {
                //std::cout << "artificial_intelligence::execute_ai_step_fly[SAVED_POINT]: FINISHED" << std::endl;
                _ai_state.sub_status = IA_ACTION_STATE_FINISHED;
            }
        } else if (_parameter == AI_ACTION_FLY_OPTION_VERTICAL_CENTER) {
            if (move_to_point(_dest_point, 0, move_speed, is_ghost, false) == true) {
...
void artificial_intelligence::execute_ai_save_point()
{
...
void artificial_intelligence::execute_ai_step_dash()
{
    if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
...
void artificial_intelligence::execute_ai_step_change_animation_type()
{
    if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
        ...
        if (started_action_timer <= TimerView::get_instance()->getTimer()) {
...
void artificial_intelligence::execute_ai_step_change_animation_type_reverse()
{
    if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
...
void artificial_intelligence::execute_ai_wait_random_time()
{
    if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
...
void artificial_intelligence::execute_ai_rotate_graphic()
{
    //std::cout << "artificial_intelligence::execute_ai_rotate_graphic::EXECUTE" << std::endl;
    if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
        ...
        //std::cout << "artificial_intelligence::execute_ai_rotate_graphic::INIT - rotation_value[" << rotation_value << "]" << std::endl;
...
bool artificial_intelligence::move_to_point(st_float_position dest_point, float speed_x, float speed_y, bool can_pass_walls, bool must_walk_along_wall)
{
...
can_move_struct artificial_intelligence::check_can_move_to_point(st_float_position dest_point, float speed_x, float speed_y, bool can_pass_walls, bool must_walk_along_wall)
{
    float xinc = 0;
    float yinc = 0;
    ...
    if (_always_move_ahead == false) {
        if ((type == AI_ACTION_WALK || type == AI_ACTION_DASH) && can_fly == false && can_move_x == true && xinc != 0 && yinc == 0) {
            ...
            int current_map_lock = GameManager::get_instance()->get_current_map_obj()->getMapPointLock(current_map_point);
    ...
    // always ahead will try to jump over obstables checking them a whole TILE ahead of time
        ...
        if (hit_ground() == true && speed_y == 0 && speed_x != 0) { // check if is trying to move on X axis only
            if ((map_lock_ahead != TERRAIN_WATER && map_lock_ahead != TERRAIN_UNBLOCKED) && (map_lock_top == TERRAIN_WATER || map_lock_top == TERRAIN_UNBLOCKED)) { // check that the terrain over the block is free
                ...
                _ai_state.sub_status = IA_ACTION_STATE_INITIAL;
                _ai_state.main_status = 0;
                //std::cout << "SET JUMP >>>>> artificial_intelligence::execute_ai_step[" << name << "] - _number: " << _number << ", _current_ai_type: " << _current_ai_type << std::endl;
...
void artificial_intelligence::randomize_x_point(int max_adjust)
{
    int rand_x = rand() % max_adjust*2;
    std::cout << ">>artificial_intelligence::randomize_x_point - max_adjust: " << max_adjust << ", start_point.x: " << start_point.x << ", rand_x: " << rand_x << std::endl;
    position.x = position.x + rand_x - max_adjust;
}

int artificial_intelligence::create_rand_x_point(int max_range)
{
...
int artificial_intelligence::create_rand_y_point(int max_range)
{
    bool keep_going = true;
    int rand_y = 0;
    while (keep_going) {
...
st_position artificial_intelligence::create_rand_point(int max_range)
{
    bool keep_going = true;
    int rand_x = 0;
    int rand_y = 0;
...
void artificial_intelligence::execute_ai_step_jump()
{
...
*/
    if (_parameter == AI_ACTION_JUMP_OPTION_TO_ROOF) {
        ...
        std::cout << "*********** artificial_intelligence::execute_ai_step_jump - extra-parameter _parameter[" << _parameter << "] not implemented ******" << std::endl;
    }

}

int artificial_intelligence::find_wall(float initial_x, int direction)
{
...
void artificial_intelligence::execute_ai_step_jump_to_wall()
{
    if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
...
void artificial_intelligence::execute_ai_replace_itself(bool morph)
{
...
void artificial_intelligence::execute_ai_step_spawn_npc()
{
...
void artificial_intelligence::execute_ai_circle_player()
{
...
void artificial_intelligence::ia_action_teleport()
{
    // go teleporting
    if (_ai_state.sub_status == IA_ACTION_STATE_INITIAL) {
...
int artificial_intelligence::get_ai_type() {
    int type = -1;
...
bool artificial_intelligence::get_is_npc()
{
    return GameData::get_instance()->get_enemy(_number)->is_npc;
}

int artificial_intelligence::get_dialog_id()
{
    return GameData::get_instance()->get_enemy(_number)->npc_dialog_id;
}


bool artificial_intelligence::always_move_ahead() const
{
...
bool artificial_intelligence::uses_fly_fall()
{
    for (int i=0; i<AI_MAX_STATES; i++) {
...
void artificial_intelligence::invert_left_right_direction()
{
    if (state.direction == ANIM_DIRECTION_LEFT) {
        set_direction(ANIM_DIRECTION_RIGHT);
    } else if (state.direction == ANIM_DIRECTION_RIGHT) {
...
bool artificial_intelligence::is_teleporting()
{
...
bool artificial_intelligence::check_moving_along_wall(int xinc, int yinc)
{
    if (xinc != 0) {
        ...
        int point_lock_top = GameManager::get_instance()->get_current_map_obj()->getMapPointLock(check_point_top);
        int point_lock_bottom = GameManager::get_instance()->get_current_map_obj()->getMapPointLock(check_point_bottom);
        ...
        int point_lock_left = GameManager::get_instance()->get_current_map_obj()->getMapPointLock(check_point_left);
        int point_lock_right = GameManager::get_instance()->get_current_map_obj()->getMapPointLock(check_point_right);
