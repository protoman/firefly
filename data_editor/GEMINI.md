# Firefly Data Editor Context

This is a C++ editor for game data of a Metroidvania-like game, similar to Shantae Half-Genie Hero.

## Project Architecture
- The build system is cmake-based.
- It uses Qt library for UI, graphics, and input.
- For map data structure it uses Tiled Map editor.
- The editor is meant for enemies, NPCs, quests, objects and more.
- It also should allow the user to place such items in the map.

## General Instructions
- When generating new code, the target language is **C++23**.
- Prioritize using modern C++ features and standard library components (e.g., `std::vector`, `std::string`, `std::unique_ptr`, `std::filesystem`).
